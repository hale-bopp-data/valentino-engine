...
export async function runVisualAuditCmd(args: string[]): Promise<void> {
    ...
    if (!source) {
        console.error('Usage: valentino visual-audit <file.html|URL> [--responsive] [--json] [--debug] [--profile auto|landing|spa|dashboard|chat|data-table|form|responsive] [--wait-for <selector>] [--timeout <ms>] [--settle <ms>] [--screenshot-dir <dir>] [--no-screenshot]');
