...
import { getProfileConfig } from './spa-profile.js';
...
export function probeRhythm(spec: PageSpecV1, options?: RhythmOptions): RhythmProbeResult {
    const profile = options?.profile ?? 'landing';
    const rules = getProfileConfig(profile).rhythmRules;
    const warnings: RhythmWarning[] = [];
    const sections = spec.sections;

    if (sections.length === 0) {
        return { valid: true, warnings, profile };
    }

    const heroIndex = sections.findIndex(s => s.type === 'hero');
    if (rules.heroFirst && heroIndex > 0) {
        warnings.push({
            index: heroIndex,
            sectionType: 'hero',
            rule: 'hero-first',
    ...
    if (rules.spacerBetweenSameSurface) {
        for (let i = 1; i < sections.length; i++) {
            const prev = sections[i - 1];
            const curr = sections[i];
